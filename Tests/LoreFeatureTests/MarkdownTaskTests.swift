...
final class MarkdownTaskTests: XCTestCase {
    ...
    func test_toggleIsRefusedWhenTheOffsetNoLongerHoldsAMarker() {
        ...
        // into prose.
        let moved = "XXXX- [ ] a\n"
        XCTAssertEqual(MarkdownDocumentModel(fullText: moved).toggle(item, in: moved), moved)
    ...
    func test_toggleIsRefusedWhenTheOffsetIsPastTheEndOfTheText() {
        let text = "- [ ] a\n"
        let item = MarkdownDocumentModel(fullText: text).taskItems[0]
        XCTAssertEqual(MarkdownDocumentModel(fullText: "-").toggle(item, in: "-"), "-")
    }

    func test_upperCaseXTogglesOff() {
        let text = "- [X] a\n"
        let model = MarkdownDocumentModel(fullText: text)
        XCTAssertEqual(model.taskItems.map(\.isChecked), [true])
        XCTAssertEqual(model.toggle(model.taskItems[0], in: text), "- [ ] a\n")
    }
    ...
    func test_malformedItemOffersNoToggle() {
        XCTAssertTrue(MarkdownDocumentModel(fullText: "- [] a\n").taskItems.isEmpty)
        XCTAssertTrue(MarkdownDocumentModel(fullText: "- a\n").taskItems.isEmpty)
        XCTAssertTrue(MarkdownDocumentModel(fullText: "- [y] a\n").taskItems.isEmpty)
    }
    ...
    func test_markerIsTheEarliestSpellingOnTheFirstLine() {
        ...
        let model = MarkdownDocumentModel(fullText: text)
        XCTAssertEqual(model.taskItems.count, 1)
        XCTAssertEqual(model.toggle(model.taskItems[0], in: text), "- [ ] see [ ] later\n")
    }

    /// `taskItems` is derived from the SAME `.checkbox` style spans the editor
    ...
    func test_taskItemsAgreeWithTheCheckboxStyleSpans() {
        ...
        XCTAssertEqual(checkboxSpans.count, model.taskItems.count)
        for (span, item) in zip(checkboxSpans, model.taskItems) {
...
final class MarkdownTaskSessionTests: XCTestCase {
    ...
    func test_toggleThroughTheEditPathAutosaves() async throws {
        ...
        let item = try XCTUnwrap(model.taskItems.first)
        engine.note.body = model.toggle(item, in: body)
        session.markChanged()
